...
AS
BEGIN	
	return(SELECT   SUM(HDCT.SoLuong * THUOC.DonGia) as N'Giá'
...
select Sum(HDCT.Soluong*THUOC.DonGia)  from HOADONCHITIET HDCT JOIN THUOC ON  HDCT.MaThuoc = THUOC.MaThuoc where MaDT = 'DT14' group by MaDT
-- 2 
...
END
GO
SELECT MaBN , SUM(GiaPhong) AS GiaPhong	
		FROM  Phong  P  JOIN BENHAN BA ON P.MaPhong = BA.MaPhong  
		GROUP BY GiaPhong  , MaBN 
...
-- THỐNG KÊ LƯỢNG THUỐC  THEO THÁNG VÀ NĂM
DROP PROC IF EXISTS SP_ThongKe_BenhNhan_theothang 
GO
CREATE PROC SP_ThongKe_BenhNhan_theothang @THANG INT 
AS
BEGIN 
SELECT BN.HoTenBN , BN.GioiTinh , BN.DiaChi ,BA.ChuanDoan , BA.HoTenNV ,P.GiaPhong AS TienKham , BN.NgayKham   FROM BENHNHAN BN INNER JOIN BENHAN BA ON BN.MaBN = BA.MaBN inner join PHONG P ON P.MaPhong = BA.MaPhong where MONTH(BN.NgayKham) =  @THANG 
END

EXEC SP_ThongKe_BenhNhan_theothang 3 
...
AS
BEGIN 
SELECT BN.HoTenBN , BN.GioiTinh , BN.DiaChi ,BA.ChuanDoan , BA.HoTenNV ,P.GiaPhong AS TienKham , BN.NgayKham   FROM BENHNHAN BN INNER JOIN BENHAN BA ON BN.MaBN = BA.MaBN inner join PHONG P ON P.MaPhong = BA.MaPhong 
END
